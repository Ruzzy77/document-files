...
def test_engine_integrates_unresolved_unit_once_and_reuses_committed_scope(
    ...
    if transport != "complete":
        ...
        assert len(scopes) == 1 and scopes[0].max_output_tokens == 3072
        assert scopes[0].reasoning_budget_tokens == (2048 if transport == "managed" else None)
...
def test_label_and_whole_line_fields_collapse_into_the_bound_value():
    ...
    assert COMPILER_VERSION == "document-files.result-compiler.v29"
