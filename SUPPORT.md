...
|---|---|---|
| 1 | **Complete primary-format extraction is not approved.** Record and note content now reach applicability, but notes self-link to their wording rather than business values. The first unit applicability request exhausts the remaining time. | Separate source ownership from applicability for note meanings; assess a note-only content path while retaining subtotal/form values. Measure the full remaining work, then verify blank interpretation, pending-only condition, units and integration without raising limits. |
