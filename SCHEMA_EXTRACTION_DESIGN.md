...
- 작성·수정일: 2026-09-08
- 상태: 1.8.0 독립 릴리스 후보 구현; 실제 모델·클라이언트 검증 진행 전
- 현재 구현 기준: Document Files 1.8.0 후보
...
제안한다. v1의 실제 공개 필드는 `document_files.api.extraction_result_schema()`를 기준으로 한다.
이 문서의 미구현 확장 제안과 현재 공개 필드를 구분한다.
...
독립 저장소와 host-independent Python facade, CLI/MCP 작업 조회·재개·삭제·진단,
짧은 저장 트랜잭션과 검증된 중간 결과 체크포인트를 추가한다. 모델 연결의 JSON 응답 기능과
출력 예산은 명시적으로 설정하며, 인증·요청 제한·시간 초과를 구분한다.

시맨틱 해석은 내부 AI가 수행하고 명시적인 값은 source binding으로 원문에서 구성한다.
원문 표기와 정규화 값을 구분하고 스키마의 개별 properties 정의에도 근거를 요구한다.
기존 v1 결과와 기본 재현 맥락 옵션은 유지한다. 실제 모델 품질은 이 경로의 단위시험과
구분하며, 미지 양식 holdout과 source-linked 결과의 독립 검토를 릴리스 근거로 남긴다.

시각 의미·긴 문서 통합·개인용 수신 AI 재현은 아직 완료 요건을 충족하지 않았다.
프로젝트 연계는 일반 스키마·시맨틱·값과 출처 계약까지이며 소프트젠의 백오피스
양식·매핑·생성 구현을 이 제품에 포함하지 않는다.
