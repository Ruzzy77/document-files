...
- 운용 대상: 프로젝트 배포는 DGX Spark(Linux ARM64, NVIDIA GB10)에서 HTTP 서버·컨테이너로
  운용하는 것을 목표로 합니다. 추론은 `accelerator: cuda` 런타임 팩으로 GPU에서 실행하고
  인식 팩은 CPU에서 실행합니다. 다섯 대상의 CPU 배포본은 계속 제공하되 GPU 품질은 별도로 확인합니다.
