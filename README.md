...
아래아한글(HWP/HWPX)과 엑셀(XLSX)을 비롯한 문서를 읽고 만들거나 편집하는
독립 플러그인이자 Python 라이브러리입니다.
...
**HWP, HWPX, XLSX, DOCX, PPTX, PDF, HTML, Markdown, TXT**를 읽습니다.
...
### 추출 결과로 문서 다시 만들기 — 아직 미지원

`reconstructionContext`는 HWPX·XLSX 등에 들어 있는 원본 XML과 그림 같은 구성 파일을
추출 결과에 보관할 수 있습니다. 다만 이 결과만으로 편집 가능한 HWPX·XLSX를 다시 만들고
내용과 배치까지 확인하는 기능은 아직 없습니다. HWP는 해당 구성 파일 보관도 지원하지 않습니다.
원본 파일을 복사하거나 기존 문서를 편집하는 기능과 구분해 주세요.
