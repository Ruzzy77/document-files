...
The neutral row positions remain in layout-first table protocol v43 / region plan
v32 / prompt v41. Correct row metadata does not establish correct interpretation;
...
Subtotal, note and unmapped value cells use a separate content child region with
...
`blank` rows bypasses column mapping without creating an empty record. Note-only
content uses the source-bound path below; subtotals and mixed content remain scalar. At least one note/subtotal row is required. Header-only tables, entirely blank
...
Mapping and table-meaning usage stay zero; content interpretation remains charged and
can fail or leave uncertainty. Skipping mapping does not mean the notes were understood.

### Source-bound content for note-only table rows

`note_content.py` handles owned rows that the accepted layout classifies exclusively
as `note` or `blank`, with at least one note. It covers a standalone nonrecord slice
and a note-only child routed from a mixed record table. Every row spanned by an owned
cell must qualify; contextual headers do not become owned rows. Subtotal, label/value,
mixed-value and unresolved rows keep their scalar value path. This is an accepted
layout decision, not a keyword rule that silently removes quantities from notes.

The `document-files.note-content.v1` request gives the complete native source inventory,
source text and relevant table context, without value bindings or an instruction to
reuse the preceding record mapping. The model must review every owned source exactly
once, select meaning kinds and literal quotations, and explicitly mark the remainder
as reviewed, unresolved or unreviewed. Repeated quotations require an occurrence index.
IDs, descriptions, paths, ranges and source bindings are derived from the original
observation, not generated by the model. Spreadsheet notes use their native text,
not the display string that prefixes a cell address. Restrictions, exceptions and
blank/zero/cache distinctions must remain in the quotations.

No fields, records or scope targets are offered by this response contract. The program
records alternate value bindings as narrative reads of the same accepted note content;
it does not create a business field merely to account for the wording. Actual meaning
and applicability can remain incomplete. An explicit unresolved or unreviewed decision
remains an issue even if quotations cover the entire source. A wrongly classified role
or a value that needs another interpretation must stay unresolved, not become an empty
successful result. This rule does not alter the native observation or the general
inline-note scope rules for scalar forms.

The compiler retains source-bound meanings with no initial applicability. Existing scope
integration later offers the actual fields, record columns, rows and document parts;
source ownership alone never applies a note to itself. A note-only region can contribute
meanings without adding business values. A content-only result's `data` is an empty
object, not proof that its meanings have been understood or linked correctly.

`noteContentState` is program/checkpoint metadata, not a generation property. It binds
the original response to the inventory hash, accepted layout and minimal parent-layout
identity. Resume validates the current parent layout history, replays literal resolution
and source accounting, and rejects changed source, scope, bindings or content. Repairs
can refine kinds, add content or resolve uncertainty but cannot lose an accepted literal
span or downgrade interpreted content. The existing scalar-local attempt bound and the
document's character, token, call and time limits remain; there is no extra free stage.
Region plan v32 / table protocol v43 / compiler v43 and the new note-content identity
reject incompatible earlier checkpoints. Public v1 contracts remain unchanged.

Direct regressions cover native notes and qualifier preservation, wrong/missing sources,
forged fields/scopes/receipts, explicit deferred review, note/subtotal routing, mixed
record-table children, repair preservation, cancellation and JSON checkpoint resume.
These establish source and contract behavior, not real-model applicability accuracy.

### Content routing, continuation and local repair
...
unreviewed; this change neither deletes its source nor approves it. Introduced in region
plan v31, this policy remains part of the current v32 checkpoint identity.
...
current table protocol is v43, including source-derived reads, layout decisions, neutral row
...
Table protocol v43, layout v3, region plan v32 and compiler v43 distinguish these
...
| Contract | Version |
|---|---|
| Semantic prompt / region plan / result compiler | v41 / v32 / v43 |
...
| Table protocol / table reference wire / table source wire / selection wire | v43 / v2 / v1 / v4 |
| Note-only source content | v1 |
